...
import { uploadFileToPinata } from "./firebase/ipfs-db";
...
export async function bundlrFund() {
  ...
  if (nodeBalance < 0.00037) await webIrys.fund(webIrys.utils.toAtomic(0.00037)); // eth
}
...
export async function contrattoProjectAddTier(inputs) {
  ...
  while (inputs["name" + i] != null) {
    ...
    const nftimgcid = await uploadFileToPinata(imgObj) //bundlrAdd(stream, { name: "Content-Type", value: "image/" + imgObj.fileExtension });
    const temp = {
      name: inputs["name" + i],
    ...
    const blob = new Blob([JSON.stringify(objs[i - 1])], { type: "application/json" });
    const uri = await uploadFileToPinata(blob)//"fake_uri" +i //await bundlrAdd(objs[i - 1], { name: "Content-Type", value: "application/json" });
    await pWithSigner.addRewardTier(uri, ethers.utils.parseUnits(inputs["price" + i].toString(), 18), parseInt(inputs["supply" + i]))

    objs[objs.length - 1].uri = uri;
